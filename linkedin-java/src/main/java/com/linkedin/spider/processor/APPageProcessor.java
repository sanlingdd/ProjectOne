...
public class APPageProcessor implements PageProcessor {
...
//ng-hide="enquiry_id"
		//names
		StaticUtil.pages +=1;
		List<String> names = page.getHtml().codes("//a[@ng-hide='enquiry_id' and @class='ng-binding']/tidyText()").all();
		List<String> ids = page.getHtml().codes("//span[@class='user_id value ng-binding']/tidyText()").all();
		List<String> chinesename = page.getHtml().codes("//span[@class='sap_name value ng-binding']/tidyText()").all();
		List<String> department = page.getHtml().codes("//span[@class='job_group value ng-binding']/tidyText()").all();
		List<String> workphone = page.getHtml().codes("//a[@class='profile_labels_phone value ng-binding']/tidyText()").all();
		List<String> mobilephone = page.getHtml().codes("//a[@class='profile_labels_mobile value ng-binding']/tidyText()").all();
		List<String> office = page.getHtml().codes("//div[@class='office ng-binding']/tidyText()").all();
		List<String> location = page.getHtml().codes("//div[@class='location ng-binding']/tidyText()").all();
		
		
		Set<APProfile> set = new HashSet<APProfile>();
		for(int iter=0; iter<30;iter++)
		{
			APProfile profile = new APProfile();
			profile.setName(names.get(iter));
			profile.setId(ids.get(iter));
			profile.setChinesename(chinesename.get(iter));
			profile.setDepartment(department.get(iter));
			profile.setWorkphone(workphone.get(iter));
			profile.setMobilephone(mobilephone.get(iter));
			profile.setOffice(office.get(iter));
			profile.setLocation(location.get(iter));
			
			StaticUtil.profiles.add(profile);
		}
