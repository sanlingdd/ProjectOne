...
public class CompanyMessageSending {
	...
	public static void HandleAPage(PageOperation obj, WebDriver driver, HuntingCompany firm) {
		...
		while (true) {
			...
			String name = driver.findElements(By.xpath(".//span[@class='name actor-name']")).get(currentNumber).getText().split(" ")[0];
			...
			if (elementsSendMessage.size() > sendedNumber) {
				try {
					...
					List<WebElement> namespans =driver.findElements(By.xpath(".//span[@class='msg-connections-typeahead__recipient-name']"));
					if(!namespans.isEmpty()){
						name = namespans.get(0).getText().split(" ")[0];						
					}
					String hintMessage = "Hi "+name+",我在帮一家专业的猎头公司做招聘,公司在世纪大道附近，老板有15年以上的从业经验，团队单产非常的高，不知道你有没有兴趣聊一聊呀？";
					WebElement messageElement = driver.findElements(By.xpath(".//div[@class='msg-form__contenteditable t-14 t-black--light t-normal flex-grow-1']")).get(0);
					messageElement.sendKeys(hintMessage);
		...
		for (HuntingCompany firm : firmsSet) {
			...
			if (firm.isLink()) {
				...
				company = "http://www.linkedin.com/search/results/people/?facetCurrentCompany=%5B" + argsStr.toString()
						+ "%5D&facetGeoRegion=%5B\"cn%3A0\"%5D&facetNetwork=%5B\"F\"%5D&origin=FACETED_SEARCH&page=10";
